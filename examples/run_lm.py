...
def load_dataset(dataset_path, task_name='rocstories', debug=False):
        ...
        for article in data['data']:
            for paragraph in article['paragraphs']:
                for qa in paragraph['qas']:
                    output.append((qa['question'].strip()))
...
def save_model(model, tokenizer, args, weights_name=WEIGHTS_NAME, override_default_weights=True):
    ...
    output_default_model_file = os.path.join(args.output_dir, WEIGHTS_NAME)
...
def main():
    ...
    print(args)
    assert args.task_name in {'rocstories', 'squad-questions-lm'}, 'Unimplemented task_name = ' + args.task_name
    ...
    if not os.path.exists(args.output_dir):
        ...
        print('Overwriting existing output directory', args.output_dir)
    ...
    train_dataset = load_dataset(args.train_dataset, args.task_name, args.debug)
    eval_dataset = load_dataset(args.eval_dataset, args.task_name, args.debug)
    ...
    nb_tr_steps, tr_loss, exp_average_loss, best_eval_loss, patience_left = 0, 0, None, float('inf'), args.patience
    model.train()
    ...
    for epoch_no in trange(int(args.num_train_epochs), desc="Epoch"):
        ...
        for step, batch in enumerate(tqdm_bar):
            ...
            if args.fp16:
                ...
                lr_this_step = args.learning_rate * warmup_linear.get_lr(nb_tr_steps, args.warmup_proportion)
            ...
            tb_writer.add_scalar('lr', lr_this_step, nb_tr_steps)
            tb_writer.add_scalar('loss', loss.item(), nb_tr_steps)
            tb_writer.add_scalar('exp_average_loss', exp_average_loss, nb_tr_steps)

        # Validation and early stopping
        eval_loss = evaluate(model, eval_dataloader, device, args, tr_loss/nb_tr_steps, epoch_no)
        if eval_loss < best_eval_loss:
            print('Best loss so far! {} -> {}'.format(best_eval_loss, eval_loss))
            best_eval_loss = eval_loss
            save_model(model, tokenizer, args, 'model_epoch_{}.bin'.format(epoch_no), override_default_weights=True)
            patience_left = args.patience
        else:
            print('Loss up from best epoch: {} -> {}'.format(best_eval_loss, eval_loss))
            save_model(model, tokenizer, args, 'model_epoch_{}.bin'.format(epoch_no), override_default_weights=False)
            patience_left -= 1
            if patience_left <= 0:
                print('Ran out of patience. Stopping training.')
                break

    print('Completed training in {}s.'.format(start_time - time.time()))


def evaluate(model, eval_dataloader, device, args, train_loss, epoch_no):
    model.eval()
    ...
    output_eval_file = os.path.join(args.output_dir, "eval_results_{}.txt".format(epoch_no))
